...
# openpyxl reads the Float fill as -999999999999.0; the canonical SoS spelling is
# -9.99999999999e+11 -- the same IEEE 754 double, kept explicit for auditability.
...
# Defaults rather than constants, so the same converter can emit a second artifact
# (e.g. from the SoS priors sheets) for a different target.
...
_DEFAULT_FILEPATH = "output/sos/{continent_id}_sword_v{number}_SOS_results.nc"
...
def check_target(module_name: str, filepath: str) -> list[str]:
    ...
    if module_name not in contracts:
        return [
            f"--module-name {module_name!r} has no contract (have: {', '.join(sorted(contracts))})"
        ]
